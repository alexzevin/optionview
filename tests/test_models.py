...
- Heston calibration: input validation, structural invariants, and round-trip accuracy with warm-started initial guesses
...
# Heston calibration
# ---------------------------------------------------------------------------
from optionview.models import calibrate_heston, HestonCalibration


class TestHestonCalibration:
    """Tests for calibrate_heston and HestonCalibration.

    Covers input validation, structural invariants on the returned dataclass,
    and round-trip accuracy using warm-started initial guesses. Warm-starting
    (initializing the optimizer at or near the true parameters) is used
    throughout to keep test runtime acceptable: Heston pricing requires
    numerical integration per quote per optimizer iteration, so convergence
    from an arbitrary starting point can take tens of seconds. From a
    warm start, the optimizer needs only a handful of iterations to confirm
    the minimum, verifying objective function wiring and fit-quality reporting
    without exercising optimizer search behavior.

    Quotes in the round-trip tests are generated by pricing with the Heston
    model at known parameters and back-solving for implied vol. The resulting
    (forward, strike, expiry_years, iv) tuples are exact up to floating-point
    precision, so the calibration objective has a true zero at the generating
    parameters.
    """

    _FORWARD: float = 100.0
    _T: float = 0.5
    _V0: float = 0.04       # initial variance: sqrt(0.04) = 20% initial vol
    _KAPPA: float = 2.0     # mean-reversion speed
    _THETA: float = 0.04    # long-run variance: 20% long-run vol
    _SIGMA: float = 0.4     # vol-of-vol
    _RHO: float = -0.7      # equity-style downward skew
    _STRIKES: list[float] = [85.0, 90.0, 95.0, 100.0, 105.0, 110.0, 115.0]
    ...
    def _make_quotes(
        ...
        """Build (forward, strike, expiry_years, iv) tuples from exact Heston prices.

        Prices each strike with heston() at rate=0 (Black-76 convention) then
        back-solves for the Black-Scholes implied vol. The resulting quotes are
        exact up to floating-point precision, giving the calibration objective
        a true zero residual at the generating parameters.
        """
        fwd = forward if forward is not None else cls._FORWARD
        t_ = t if t is not None else cls._T
        v0_ = v0 if v0 is not None else cls._V0
        kappa_ = kappa if kappa is not None else cls._KAPPA
        theta_ = theta if theta is not None else cls._THETA
        sigma_ = sigma if sigma is not None else cls._SIGMA
        rho_ = rho if rho is not None else cls._RHO
        ks = strikes if strikes is not None else cls._STRIKES
        quotes: list[tuple[float, float, float, float]] = []
        for k in ks:
            price = heston(
                spot=fwd, strike=k, rate=0.0, expiry_years=t_,
                v0=v0_, kappa=kappa_, theta=theta_, sigma=sigma_, rho=rho_,
                option_type="call",
            )
            iv = implied_volatility(price, fwd, k, 0.0, t_, "call")
            quotes.append((fwd, k, t_, iv))
        return quotes
    ...
    def test_invalid_forward_in_quote_raises(self) -> None:
        """A quote with forward <= 0 raises ValueError."""
        with pytest.raises(ValueError):
            calibrate_heston([(0.0, 100.0, 0.5, 0.25)])

    def test_invalid_strike_in_quote_raises(self) -> None:
        """A quote with strike <= 0 raises ValueError."""
        with pytest.raises(ValueError):
            calibrate_heston([(100.0, -5.0, 0.5, 0.25)])

    def test_invalid_expiry_in_quote_raises(self) -> None:
        """A quote with expiry_years <= 0 raises ValueError."""
        with pytest.raises(ValueError):
            calibrate_heston([(100.0, 100.0, 0.0, 0.25)])

    def test_invalid_iv_in_quote_raises(self) -> None:
        """A quote with market_iv <= 0 raises ValueError."""
        with pytest.raises(ValueError):
            calibrate_heston([(100.0, 100.0, 0.5, 0.0)])

    # ------------------------------------------------------------------
    # Structural invariants
    # ------------------------------------------------------------------

    def test_return_type_is_heston_calibration(self) -> None:
        """calibrate_heston returns a HestonCalibration dataclass instance."""
        result = calibrate_heston(
            self._make_quotes(),
            v0_init=self._V0, kappa_init=self._KAPPA,
            theta_init=self._THETA, sigma_init=self._SIGMA, rho_init=self._RHO,
        )
        assert isinstance(result, HestonCalibration)

    def test_n_points_matches_quote_count(self) -> None:
        """fit.n_points equals the number of market quotes supplied."""
        quotes = self._make_quotes()
    ...
    def test_max_abs_error_ge_rmse(self) -> None:
        """max_abs_error is always >= RMSE: the maximum dominates the RMS average."""
        fit = calibrate_heston(
            self._make_quotes(),
            v0_init=self._V0, kappa_init=self._KAPPA,
            theta_init=self._THETA, sigma_init=self._SIGMA, rho_init=self._RHO,
    ...
    def test_fitted_v0_positive(self) -> None:
        """Fitted v0 is strictly positive, enforced by optimizer bounds."""
        fit = calibrate_heston(
            self._make_quotes(),
            v0_init=self._V0, kappa_init=self._KAPPA,
            theta_init=self._THETA, sigma_init=self._SIGMA, rho_init=self._RHO,
    ...
    def test_fitted_kappa_positive(self) -> None:
        """Fitted kappa is strictly positive, enforced by optimizer bounds."""
        fit = calibrate_heston(
            self._make_quotes(),
            v0_init=self._V0, kappa_init=self._KAPPA,
            theta_init=self._THETA, sigma_init=self._SIGMA, rho_init=self._RHO,
    ...
    def test_fitted_theta_positive(self) -> None:
        """Fitted theta is strictly positive, enforced by optimizer bounds."""
        fit = calibrate_heston(
            self._make_quotes(),
            v0_init=self._V0, kappa_init=self._KAPPA,
            theta_init=self._THETA, sigma_init=self._SIGMA, rho_init=self._RHO,
    ...
    def test_fitted_sigma_positive(self) -> None:
        """Fitted sigma is strictly positive, enforced by optimizer bounds."""
        fit = calibrate_heston(
            self._make_quotes(),
            v0_init=self._V0, kappa_init=self._KAPPA,
            theta_init=self._THETA, sigma_init=self._SIGMA, rho_init=self._RHO,
    ...
    def test_fitted_rho_in_unit_interval(self) -> None:
        """Fitted rho lies strictly inside (-1, 1)."""
        fit = calibrate_heston(
            self._make_quotes(),
            v0_init=self._V0, kappa_init=self._KAPPA,
    ...
    def test_feller_satisfied_is_bool(self) -> None:
        """feller_satisfied is a plain Python bool, not a numpy bool or int."""
        fit = calibrate_heston(
            self._make_quotes(),
            v0_init=self._V0, kappa_init=self._KAPPA,
            theta_init=self._THETA, sigma_init=self._SIGMA, rho_init=self._RHO,
        )
        assert isinstance(fit.feller_satisfied, bool)

    def test_feller_flag_consistent_with_parameters(self) -> None:
        """feller_satisfied == (2 * kappa * theta > sigma^2) for the fitted parameters.

        The Feller condition determines whether the variance process can reach zero.
        The flag is derived from the fitted values, so it must be algebraically
        consistent with them: if an independent calculation of the condition produces
        a different result, the flag was set incorrectly.
        """
        fit = calibrate_heston(
            self._make_quotes(),
            v0_init=self._V0, kappa_init=self._KAPPA,
        ...
        expected = 2.0 * fit.kappa * fit.theta > fit.sigma ** 2
        assert fit.feller_satisfied == expected

    # ------------------------------------------------------------------
    # Round-trip accuracy
    # ------------------------------------------------------------------

    def test_warm_start_recovers_low_rmse(self) -> None:
        """Calibrating to exact Heston IVs with a warm start yields near-zero RMSE.

        Quotes generated from known Heston parameters form a zero-residual surface.
        Initializing the optimizer at the true parameters confirms that the objective
        function, parameter bounds, and IV-error reporting pipeline are all correct.
        RMSE below 1e-3 (0.1 vol point) is the acceptance threshold; the optimizer
        starting exactly at the minimum should land far below this in practice.
        """
        quotes = self._make_quotes()
        fit = calibrate_heston(
            quotes,
            v0_init=self._V0,
            kappa_init=self._KAPPA,
            theta_init=self._THETA,
            sigma_init=self._SIGMA,
            rho_init=self._RHO,
        )
        assert fit.rmse < 1e-3, (
            f"Expected RMSE < 1e-3 on warm-started exact Heston quotes, "
            f"got {fit.rmse:.6f}. Check that heston() and the calibration "
            f"objective are consistent."
        )

    def test_negative_rho_fitted_for_downward_skew(self) -> None:
        """Quotes generated with rho=-0.7 (equity skew) produce a negative fitted rho.

        With a negative rho the Heston model generates a downward-sloping implied vol
        smile: OTM puts are richer than OTM calls. The optimizer must recognize this
        shape and converge to a negative rho. A positive result would indicate a sign
        convention error or a badly conditioned objective.
        """
        quotes = self._make_quotes(rho=-0.7)
        fit = calibrate_heston(
            quotes,
            v0_init=self._V0, kappa_init=self._KAPPA,
            theta_init=self._THETA, sigma_init=self._SIGMA, rho_init=self._RHO,
        ...
        assert fit.rho < 0.0, (
            f"Expected negative rho for downward-skew quotes, got rho={fit.rho:.4f}"
        )

    def test_positive_rho_fitted_for_upward_skew(self) -> None:
        """Quotes generated with rho=+0.5 (upward skew) produce a positive fitted rho.

        A positive rho makes OTM calls richer than OTM puts, which is the opposite
        of the equity skew and common in some commodity markets. The fitted rho
        must match the sign of the input to confirm the optimizer is sensitive to
        skew direction.
        """
        quotes = self._make_quotes(rho=0.5)
        fit = calibrate_heston(
            quotes,
            v0_init=self._V0, kappa_init=self._KAPPA,
            theta_init=self._THETA, sigma_init=self._SIGMA, rho_init=0.3,
        )
        assert fit.rho > 0.0, (
            f"Expected positive rho for upward-skew quotes, got rho={fit.rho:.4f}"
        )
